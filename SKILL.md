...
| Engulf | Linux apps that open `/dev/video*` themselves: `bwrap` or `LD_PRELOAD`. PipeWire is not engulfed. Windows 11 build 22000+ can register a user-mode camera (friendly name VeilLock; the picker adds Windows Virtual Camera) while `veilcam-register.exe` is running. VeilLock does not hook. Other cameras remain. The microphone is still **CABLE Output** if VB-Cable is installed. macOS (SIP, hardened runtime, Apple-signed FaceTime) and iOS are not engulfed. Chromium: the extension wraps `getUserMedia`. |

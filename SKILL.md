...
| Loopback desk | `veillock ui` on 127.0.0.1:8761 plans join and engulf through the same adapter as the CLI and does not launch them. Seal and play in memory is the AES-256-GCM recording demo. The layer map is [docs/layers.md](docs/layers.md). The human UI is aziel-runtime. `suite/runtime-ui.json` is schema `veillock-runtime-ui-1`. Public door ops stay empty. ACT-RECEIPT-1.0 `hash` / `request` / `output` / `event` are owned by aziel-runtime; this package does not append that chain. |

...
| Loopback desk | `veillock ui` on 127.0.0.1:8761 plans join and engulf through the same adapter as the CLI and does not launch them. Seal and play in memory is the AES-256-GCM recording demo. The layer map is [docs/layers.md](docs/layers.md). `suite/azinterface-tile.json` is the AZInterface handoff. VeilLock is not merged into AZInterface. |
